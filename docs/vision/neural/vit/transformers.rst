...
Fundamentos
-----------
Si trataramos a una imagen como una secuencia de pixeles, podríamos pensar en aplicar la arquitectura de transformers directamente sobre los pixeles de las imágenes. Sin embargo, esto requeriría que cada pixels pueda "prestar atención" (attend) a cada uno de los pixeles restantes. Es conocido que las arquitecturas de atención escalan cuadraticamente con respecto al tamaño de la entrada, lo que hace esta técnica inviable en el mundo de imágenes. Entonces, para poder utilizar imágenes dentro de una arquitectura basada en :doc:`/nlp/neural/transformers`, las imágenes son segmentadas (o "divididas") en pequeños grupos de pixeles, llamados parches, los cuales son linearmente proyectados a las dimensiones de entrada que la arquitectura del transformer utiliza. La secuencia de parches resultante es injectada en un transformer tradicional. 
...
Una imagen de dimensiones HxWxC resulta en una secuencia de :math:`N = HW / {P ^ 2}` tokens de tamaño :math:`PxP` (es decir, la resolución de cada uno de los tokens - o parches). Cada uno de estos parches luego es mapeado a un espacio vectorial de dimension D. Asi entonces, cada parche de 2 dimensiones (PxP) es transformado a 1 dimensión (*D*) a traves de una función que es aprendida y que es constante en toda la arquitectura. Los embeddings en la dimensión *D* se los conoce como **patch embeddings**.

.. figure:: ../../_images/state_vit_architecture_embeddings.png
  :alt: Arquitectura de embeddings.
